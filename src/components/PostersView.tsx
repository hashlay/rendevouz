...
function getDefaultThemeConfig(themeIdx: number = 0): any {
  ...
  if (themeIdx === 2) {
    return {
      ...
      campusNameY: 70,
      campusNameSize: 28,
      campusNameColor: '#4A2E2B',
      campusNameFont: 'sans-serif',
      showCampusName: false,
      festNameX: 540,
      festNameY: 120,
      festNameSize: 36,
      festNameColor: '#8C6B1C',
      festNameFont: 'sans-serif',
      showFestName: false,
      winnerSize: 34,
      unitSize: 22,
...
export default function PostersView({ user, token, eventSettings, onSettingsUpdated }: PosterGeneratorViewProps) {
  ...
  const existingThemes = Array.isArray(rawTemplateConfig.customThemes) ? rawTemplateConfig.customThemes : [];
  const migratedConfig = migrateOldConfig({
    ...rawTemplateConfig,
    customThemes: defaultThemes.map((dt, idx) => existingThemes[idx] || dt)
  }, defaultThemes);
