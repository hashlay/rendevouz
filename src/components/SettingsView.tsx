...
export default function SettingsView({ user, token, eventSettings }: SettingsViewProps) {
  ...
  const [globalPoints4, setGlobalPoints4] = useState(0);
  const [globalPoints5, setGlobalPoints5] = useState(0);
  const [globalPoints6, setGlobalPoints6] = useState(0);
  const [globalPoints7, setGlobalPoints7] = useState(0);
  const [globalPoints8, setGlobalPoints8] = useState(0);
  const [globalPoints9, setGlobalPoints9] = useState(0);
  const [globalPoints10, setGlobalPoints10] = useState(0);
  const [entityMode, setEntityMode] = useState<'unit' | 'house' | 'team'>('team');
  const [autoRemoveLogoBg, setAutoRemoveLogoBg] = useState(false);
  const [fillLogo, setFillLogo] = useState(true);
  ...
  const fetchSettingsAndUnits = async () => {
    ...
    try {
      ...
      setGlobalPoints4(data.globalPointsRank4 ?? 0);
      setGlobalPoints5(data.globalPointsRank5 ?? 0);
      setGlobalPoints6(data.globalPointsRank6 ?? 0);
      setGlobalPoints7(data.globalPointsRank7 ?? 0);
      setGlobalPoints8(data.globalPointsRank8 ?? 0);
      setGlobalPoints9(data.globalPointsRank9 ?? 0);
      setGlobalPoints10(data.globalPointsRank10 ?? 0);
      setEntityMode(data.entityMode || 'team');
