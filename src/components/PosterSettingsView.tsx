...
export default function PosterSettingsView({ user, token, eventSettings, onSettingsUpdated }: PosterSettingsViewProps) {
  ...
  // Helper to ensure 2 themes
  const normalizeThemesAndConfigs = (themesIn: any, configsIn: any) => {
    let themes = [
      (Array.isArray(themesIn) && themesIn[0]) ? themesIn[0] : defaultThemes[0],
      defaultThemes[1]
    ];
    const configs = { ...(configsIn || {}) };
    if (!configs[0]) configs[0] = getDefaultThemeConfig(0);
    if (!configs[1]) configs[1] = getDefaultThemeConfig(1);
    delete configs[2];
    delete configs[3];
  ...
  const updateConf = (key: string, value: any) => {
    setThemeConfigs((prev: any) => ({
      ...prev,
      [selectedThemeIndex]: {
        ...getThemeConfig(selectedThemeIndex),
